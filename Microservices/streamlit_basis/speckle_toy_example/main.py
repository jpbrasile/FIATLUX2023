...
Page Config: The title and icon of the Streamlit page are set.

Containers: The page is divided into several containers (header, input, viewer, report, and graphs) using st.container().

Header: The header of the page is created, with a title "Speckle Stream Activity App📈". An expandable section is added to the header that provides information about the app.

Inputs: In this section, the user inputs the Speckle server URL and token. The user also selects the stream they want to view from a dropdown list of streams.

Client: A SpeckleClient is created using the Speckle server URL provided by the user. The user's account is retrieved from the token, and the client is authenticated using the user's account.

Stream Info: Information about the selected stream, including its branches and commits, is retrieved.

Commit2Viewer Function: A function named commit2viewer is defined that generates an iframe from the commit id.

Viewer: The latest commit of the selected stream is displayed in an iframe using the commit2viewer function.

View Button: A button is added to the viewer section to allow the user to view the 3D model of the stream.
"""
...
if view:
    ...
    my_glass="""<iframe src="https://speckle.xyz/embed?stream="""+str(stream.id)+"""" width="600" height="400" frameborder="0"></iframe>"""
